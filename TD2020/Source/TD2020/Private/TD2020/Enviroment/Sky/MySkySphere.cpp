...
AMySkySphere::AMySkySphere(const FObjectInitializer& ObjectInitializer)
{
    ...
    static ConstructorHelpers::FObjectFinder<UStaticMesh> SphereAsset(
        TEXT("/Game/TD2020/Materials/Sky/SM_SkySphere.SM_SkySphere"));
    ...
    static ConstructorHelpers::FObjectFinder<UMaterial> MatObj(
        TEXT("Material'/Game/TD2020/Materials/Sky/M_Sky_Panning_Clouds2.M_Sky_Panning_Clouds2'"));
    ...
    static ConstructorHelpers::FObjectFinder<UCurveLinearColor> HorizonColorCurve_Helper(
        TEXT("/Game/TD2020/Textures/Sky/C_Sky_Horizon_Color.C_Sky_Horizon_Color"));
    static ConstructorHelpers::FObjectFinder<UCurveLinearColor> ZenithColorCurve_Helper(
        TEXT("/Game/TD2020/Textures/Sky/C_Sky_Zenith_Color.C_Sky_Zenith_Color"));
    static ConstructorHelpers::FObjectFinder<UCurveLinearColor> CloudColorCurve_Helper(
        TEXT("/Game/TD2020/Textures/Sky/C_Sky_Cloud_Color.C_Sky_Cloud_Color"));
...
void AMySkySphere::OnConstruction(const FTransform& Transform)
{
        ...
        SkyMaterialInstance->SetVectorParameterValue("Light direction",
                                                     FLinearColor(
                                                         UKismetMathLibrary::Conv_RotatorToVector(
                                                             DirectionalLightActor->GetActorRotation())));
        SkyMaterialInstance->SetVectorParameterValue("Sun color",
                                                     FLinearColor(
                                                         DirectionalLightActor->GetLightComponent()->GetLightColor()));
        SunHeight = UKismetMathLibrary::MapRangeUnclamped(DirectionalLightActor->GetActorRotation().Pitch, 0.0f, -18.0f,
                                                          0.0f, 1.0f);
    }
    else
    {
        ...
        SkyMaterialInstance->SetVectorParameterValue("Sun color",
                                                     UKismetMathLibrary::LinearColorLerp(
                                                         ...
                                                         FLinearColor(1.0f, 0.9f, 0.744f, 1.0f),
                                                         UKismetMathLibrary::FClamp(SunHeight + 0.2f, 0.0f, 1.0f)));
    }

    if (ColorsDeterminedBySunPosition)
    {
        SkyMaterialInstance->
            SetVectorParameterValue("Horizon color", HorizonColorCurve->GetLinearColorValue(SunHeight));
        ...
        SkyMaterialInstance->SetScalarParameterValue("Horizon falloff",
                                                     UKismetMathLibrary::Lerp(
                                                         3.0f, 7.0f, UKismetMathLibrary::Abs(SunHeight)));
    }
    else
    {
    ...
    SkyMaterialInstance->SetScalarParameterValue("Sun height",
                                                 UKismetMathLibrary::SelectFloat(
                                                     UKismetMathLibrary::Abs(SunHeight), 0.0f, SunHeight < 0.0f));
...
bool AMySkySphere::UpdateSunDirection()
{
    if (!Valid)
    {
        GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red,
                                         TEXT(
                                             "INVALID INIT MY SKY SPHERE - unable to update sky material properties and sun height"));
        return false;
    }
...
void AMySkySphere::SetCloudSpeed(float CloudSpeedIn)
{
    CloudSpeed = CloudSpeedIn;
    SkyMaterialInstance->SetScalarParameterValue("Cloud speed", CloudSpeedIn);
}
