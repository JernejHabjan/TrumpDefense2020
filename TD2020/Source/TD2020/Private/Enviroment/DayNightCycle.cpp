...
ADayNightCycle::ADayNightCycle()
{
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> MoonAsset(TEXT("StaticMesh'/Game/TD2020/Meshes/Meshes/MaterialSphere.MaterialSphere'"));
	...
	static ConstructorHelpers::FObjectFinder<UMaterial> MatObj(TEXT("Material'/Game/TD2020/Materials/Sky/M_Moon.M_Moon'"));
	MoonMaterial = MatObj.Object;
	static ConstructorHelpers::FObjectFinder<UCurveLinearColor> SkyColorCurve_Helper(TEXT("CurveLinearColor'/Game/TD2020/Textures/Sky/C_Sky_Zenith_Color.C_Sky_Zenith_Color'"));
	SkyColor = SkyColorCurve_Helper.Object;


	//references - Set them in editor in world outliner
	SunLight = nullptr; 
	MoonLight = nullptr;
	SkySphere = nullptr;
	GameStateRef = nullptr; 

	//static ConstructorHelpers::FObjectFinder <UClass> MyGameState_BP_Helper(TEXT("Blueprint'/Game/TD2020/Blueprints/Logics/MyGameState_BP.MyGameState_BP_C'"));
	//if (MyGameState_BP_Helper.Succeeded()) {
	//	GameStateRef_BP = MyGameState_BP_Helper.Object;
	//}
...
void ADayNightCycle::OnConstruction(const FTransform& Transform) {
	...
	GameStateRef = Cast<AMyGameState>(GetWorld()->GetGameState()); // TODO ----------------------------------- THIS GAME STATE IS NOT THE SAME AS ITS IN EDITOR
	...
	if (EarthLike) {
		...
		// from february
		if (Month >= 2) {
			for (int i = 1; i < Month - 1; i++) { // -1 because for current month you dont have to count days
	...
	// we offset it by 11 days because it starts on 1. of january but max declination is on 21. December
	SunDeclination = UKismetMathLibrary::Cos(UKismetMathLibrary::MultiplyByPi(((SunDeclinationDrive + 11.0f) / SunYearDuration) * 2.0f)) * MaxDeclination;
	...
	MoonDeclination = UKismetMathLibrary::Cos(UKismetMathLibrary::MultiplyByPi((MoonDeclinationDrive / MoonYearDuration) * 2.0f)) * MaxDeclination;
	// It takes 244 circles for inclination to finish its cycle
	// 5.4 is the 5.4 degrees of moon tilt
	MoonInclination = UKismetMathLibrary::Cos(UKismetMathLibrary::MultiplyByPi((MoonDeclinationDrive / MoonYearDuration / 244.0f) * 2.0f)) * 5.4;
...
void ADayNightCycle::Tick(float DeltaTime)
{
	...
	if (!Valid) {
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("INVALID INIT DAY NIGHT CYCLE OR MY SKY SPHERE INIT"));
		PrimaryActorTick.bCanEverTick = false;
		return;
	}
...
void ADayNightCycle::SunTrajectory()
{
	...
	if (SunPolar) {
		...
		SunPitch = (UKismetMathLibrary::Square(((SunDrive - Shift) / (SunDayDuration * 0.25f)) *  UKismetMathLibrary::Sqrt(90.0f - UKismetMathLibrary::Abs(Latitude))) * DayNightFlip) - (DayNightFlip * MaxSunAngleHorizon);
		...
		SunHalfDayLight = UKismetMathLibrary::Acos(UKismetMathLibrary::DegTan(Latitude) * UKismetMathLibrary::DegTan(-SunDeclination)) / (6.28319f / SunDayDuration);

		//sun half night -from noon to night
		SunHalfNight = UKismetMathLibrary::Acos(UKismetMathLibrary::DegTan(Latitude) * UKismetMathLibrary::DegTan(SunDeclination)) / (6.28319f / SunDayDuration);
		...
		SunPitch = (UKismetMathLibrary::Square(((SunDrive - Shift) / Length) *  UKismetMathLibrary::Sqrt(MaxSunAngleHorizon)) * DayNightFlip) - (DayNightFlip * MaxSunAngleHorizon);
...
void ADayNightCycle::MoonTrajectory()
{
	...
	MoonHalfDayLight = UKismetMathLibrary::Acos(UKismetMathLibrary::DegTan(Latitude) * UKismetMathLibrary::DegTan(-(MoonDeclination + MoonInclination))) / (6.28319f / MoonDayDuration);

	//Moon half night -from noon to night
	MoonHalfNight = UKismetMathLibrary::Acos(UKismetMathLibrary::DegTan(Latitude) * UKismetMathLibrary::DegTan(MoonDeclination + MoonInclination)) / (6.28319f / MoonDayDuration);
	...
	float MaxMoonAngleHorizon = 90.0f - UKismetMathLibrary::Abs(Latitude - (MoonDeclination * MoonDayNightFlip + MoonInclination * MoonDayNightFlip));

	MoonPitch = (UKismetMathLibrary::Square(((MoonDrive - MoonShift) / MoonLength) *  UKismetMathLibrary::Sqrt(MaxMoonAngleHorizon)) * MoonDayNightFlip) - (MoonDayNightFlip * MaxMoonAngleHorizon);
...
void ADayNightCycle::Clock()
{
	...
	GameTime = { Seconds, Minutes, Hours };
	FString GameTimeString = FString::FromInt(GameTime[0]) + " " + FString::FromInt(GameTime[1]) + " " + FString::FromInt(GameTime[2]);
...
void ADayNightCycle::Calendar()
{
	...
	FString GameDateString = FString::FromInt(GameDate[0]) + " " + FString::FromInt(GameDate[1]) + " " + FString::FromInt(GameDate[2]);
...
bool ADayNightCycle::Validate()
{
	...
	return (
		...
		//GameStateRef->IsValidLowLevel() &&
		//GameStateRef_BP->IsValidLowLevel()
		);
