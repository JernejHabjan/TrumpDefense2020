...
void UFunctionLibrary::SetClockwork(const float DeltaSeconds, const float TimeUnit, const float ClockworkIn, float & ClockworkOut, int & DayTick)
{
...
void UFunctionLibrary::CalculateSlope(AActor* Actor, const FVector Extent, bool & BadLocation)
{
	...
	if (IndexOfMinValue > -1 && IndexOfMaxValue > -1) {
		...
		if (Actor->GetWorld()->SweepSingleByChannel(HitOut, Start, End, Rotation, TraceChannel, FCollisionShape::MakeBox(Shape), TraceParams)) {
	...
	BadLocation = UKismetMathLibrary::Abs(MinValue) > MaxDifference || UKismetMathLibrary::Abs(MaxValue) > MaxDifference;

}

void UFunctionLibrary::GetVectors(const AActor * Actor, FVector & ActorLocation, FVector & ForwardVector, FVector & RightVector)
{
...
void UFunctionLibrary::SlopeTrace(const AActor * Actor, const FVector TraceLocation, float & Difference)
{
	...
	Actor->GetWorld()->LineTraceSingleByChannel(OutHit, TraceLocation + FVector(0.0f, 0.0f, 1000.0f), TraceLocation - FVector(0.0f, 0.0f, 1000.0f), COLLISION_LANDSCAPE, RV_TraceParams, FCollisionResponseParams());
	Difference = Actor->GetActorLocation().Z - OutHit.ImpactPoint.Z;
	

}

void UFunctionLibrary::HUD_LerpLocation(const FVector CurrentLocation, const FVector DestinationLocation, const float Speed, FVector & NewLocation)
{
