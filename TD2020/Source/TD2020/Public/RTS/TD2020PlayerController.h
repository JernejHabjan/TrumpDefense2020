...
class ATD2020BuildingCursor;
class ATD2020CameraBoundsVolume;
class UTD2020PlayerAdvantageComponent;
...
/**
 * Player controller with TD2020 features, such as selection and mouse camera movement.
 */
UCLASS()
class TD2020_API ATD2020PlayerController : public APlayerController
{
	GENERATED_BODY()
	
public:
    /** Movement speed of the camera when moved with keys or mouse, in cm/sec. */
    UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "TD2020|Camera", meta = (ClampMin = 0))
    float CameraSpeed;
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "TD2020|Construction")
	TSubclassOf<ATD2020BuildingCursor> BuildingCursorClass;


    ATD2020PlayerController();
	...
	ATD2020TeamInfo* GetTeamInfo() const;
	 ...
	 */
	UFUNCTION(BlueprintNativeEvent, Category = "TD2020", meta = (DisplayName = "CanPlaceBuilding"))
	...
	virtual void NotifyOnVisionInfoAvailable(ATD2020VisionInfo* VisionInfo);

	/** Event when this player is now owning the specified actor. */
	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Ownership", meta = (DisplayName = "OnActorOwnerChanged"))
	void ReceiveOnActorOwnerChanged(AActor* Actor);

	/** Event when the player begins placing a building. */
	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Construction", meta = (DisplayName = "OnBuildingPlacementStarted"))
	void ReceiveOnBuildingPlacementStarted(TSubclassOf<AActor> BuildingClass);

	/** Event when the player confirms a location for placing a building. */
	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Construction", meta = (DisplayName = "OnBuildingPlacementConfirmed"))
	void ReceiveOnBuildingPlacementConfirmed(TSubclassOf<AActor> BuildingClass, const FVector& Location);

	/** Event when the player receives an error placing a building at a specific location. */
	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Construction", meta = (DisplayName = "OnBuildingPlacementError"))
	void ReceiveOnBuildingPlacementError(TSubclassOf<AActor> BuildingClass, const FVector& Location);

	/** Event when the player cancels placing a building. */
	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Construction", meta = (DisplayName = "OnBuildingPlacementCancelled"))
	void ReceiveOnBuildingPlacementCancelled(TSubclassOf<AActor> BuildingClass);

    /** Event when an error has occurred that can be presented to the user. */
    UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Error", meta = (DisplayName = "OnErrorOccurred"))
    void ReceiveOnErrorOccurred(const FString& ErrorMessage);

	/** Event when an actor has received an attack order. */
	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Orders", meta = (DisplayName = "OnIssuedAttackOrder"))
	void ReceiveOnIssuedAttackOrder(APawn* OrderedPawn, AActor* Target);

	/** Event when an actor has received a begin construction order. */
	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Orders", meta = (DisplayName = "OnIssuedBeginConstructionOrder"))
	...
	/** Event when an actor has received a continue construction order. */
	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Orders", meta = (DisplayName = "OnIssuedContinueConstructionOrder"))
	void ReceiveOnIssuedContinueConstructionOrder(APawn* OrderedPawn, AActor* ConstructionSite);

	/** Event when an actor has received a gather order. */
	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Orders", meta = (DisplayName = "OnIssuedGatherOrder"))
	void ReceiveOnIssuedGatherOrder(APawn* OrderedPawn, AActor* ResourceSource);

    /** Event when an actor has received a move order. */
    UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Orders", meta = (DisplayName = "OnIssuedMoveOrder"))
    void ReceiveOnIssuedMoveOrder(APawn* OrderedPawn, const FVector& TargetLocation);

    /** Event when an actor has received a production order. */
    UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Orders", meta = (DisplayName = "OnIssuedProductionOrder"))
    void ReceiveOnIssuedProductionOrder(AActor* OrderedActor, int32 ProductIndex);

	/** Event when an actor has received a stop order. */
	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Orders", meta = (DisplayName = "OnIssuedStopOrder"))
	void ReceiveOnIssuedStopOrder(APawn* OrderedPawn);

	/** Event when the player has clicked a spot on the minimap. */
	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Minimap", meta = (DisplayName = "OnMinimapClicked"))
    ...
    /** Event when the set of selected actors of this player has changed. */
    UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Selection", meta = (DisplayName = "OnSelectionChanged"))
	...
	/** Event when vision info for this player has been replicated and is available. */
	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Vision", meta = (DisplayName = "OnVisionInfoAvailable"))
	void ReceiveOnVisionInfoAvailable(ATD2020VisionInfo* VisionInfo);

	UFUNCTION(BlueprintImplementableEvent, Category = "TD2020|Vision", meta = (DisplayName = "MinimapLeftClick"))
	void MinimapLeftClick(const FPointerEvent& InMouseEvent, const FVector2D& MinimapPosition, const FVector& WorldPosition);
...
private:
    ...
    ATD2020CameraBoundsVolume* CameraBoundsVolume;
	...
	ATD2020BuildingCursor* BuildingCursor;
    ...
    UTD2020PlayerAdvantageComponent* PlayerAdvantageComponent;
    ...
    UFUNCTION()
    void StaTD2020electActors();

	/** Selects all selectable actors within the created selection frame, started by StaTD2020electActors. */
	UFUNCTION()
	void FinishSelectActors();

	/** Force showing all construction progress bars. */
	UFUNCTION()
	void StaTD2020howingConstructionProgressBars();
	...
	UFUNCTION()
	void StaTD2020howingHealthBars();
	...
	UFUNCTION()
	void StaTD2020howingProductionProgressBars();
