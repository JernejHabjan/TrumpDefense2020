...
public:
	...
	// OUT int Day Tick
	UFUNCTION(Category = FunctionLibrary, BlueprintPure)
	static void SetClockwork(const float DeltaSeconds, const float TimeUnit, const float ClockworkIn, float &ClockworkOut, int &DayTick);
	...
	UFUNCTION(Category = FunctionLibrary, BlueprintPure)
	static void HUD_LerpLocation(const FVector CurrentLocation, const FVector DestinationLocation, const float Speed, FVector &NewLocation);
